...
while getopts ':n:i:r:g:f:c:m:ba:h' OPTION; do
    case "${OPTION}" in
        n)
            n=${OPTARG}
            ;;
...
if [ -z "${n}" ] || [ -z "${i}" ] || [ -z "${r}" ] || [ -z "${g}" ] || [ -z "${f}" ]; then
    usage
fi

shift $(($OPTIND -1))

core_pair="${i}/core_pair_summary.tsv"
low_freq="${i}/low_frequency_gene_placement.tsv"

# Find all core pairs with max_acc > cutoff
csvtk filter -t -f "max_acc>=${n}" ${core_pair} | csvtk del-header | csvtk tab2csv > filtered_core_pair.csv
